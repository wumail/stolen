...
这个 `stoneGame` 函数怎么写呢？博弈问题的难点在于，两个人要轮流进行选择，而且都贼精明，应该如何编程表示这个过程呢？其实不难，还是按照 [动态规划核心框架](动态规划详解进阶.md) 中强调多次的套路，首先明确 `dp` 数组的含义，然后只要找到「状态」和「选择」，一切就水到渠成了。
...
根据前文 [动态规划答疑篇](最优子结构.md) 判断 `dp` 数组遍历方向的原则，算法应该倒着遍历 `dp` 数组：
