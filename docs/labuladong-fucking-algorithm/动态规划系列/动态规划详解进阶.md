...
上述例子就相当于把 DP table 的大小从 `n` 缩小到 2。我会在后文 [对动态规划发动降维打击](状态压缩技巧.md) 进一步讲解这个压缩空间复杂度的技巧，一般来说用来把一个二维的 DP table 压缩成一维，即把空间复杂度从 O(n^2) 压缩到 O(n)。
...
> tip：关于最优子结构的问题，后文 [动态规划答疑篇](最优子结构.md) 还会再举例探讨。
...
* [动态规划设计：最长递增子序列](动态规划设计：最长递增子序列.md)
