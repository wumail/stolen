...
本文是两年前发的 [动态规划答疑篇](https://mp.weixin.qq.com/s/qvlfyKBiXVX7CCwWFR-XKg) 的修订版，根据我的不断学习总结以及读者的评论反馈，我给扩展了更多内容，力求使本文成为继 [动态规划核心套路框架](动态规划详解进阶.md) 之后的一篇全面答疑文章。以下是正文。
...
这次我给你提出的问题就**不符合最优子结构**，因为你没办通过每个班的最优值推出全校的最优值，没办法通过子问题的最优值推出规模更大的问题的最优值。前文 [动态规划详解](动态规划详解进阶.md) 说过，想满足最优子结，子问题之间必须互相独立。全校的最大分数差可能出现在两个班之间，显然子问题不独立，所以这个问题本身不符合最优子结构。
...
前文不 [同定义不同解法](动态规划之四键键盘.md) 和 [高楼扔鸡蛋问题](高楼扔鸡蛋问题.md) 就展示了如何改造问题，不同的最优子结构，可能导致不同的解法和效率。
...
看了前文 [动态规划核心套路](动态规划详解进阶.md)，我知道了如何一步步优化动态规划问题；

看了前文 [动态规划设计：数学归纳法](动态规划设计：最长递增子序列.md)，我知道了利用数学归纳法写出暴力解（状态转移方程）。
...
比如最简单的例子，[动态规划核心套路](动态规划详解进阶.md) 中斐波那契数列的递归树：
...
再举个稍微复杂的例子，前文 [正则表达式问题](动态规划之正则表达.md) 的暴力解代码：
...
比如说前文 [编辑距离问题](编辑距离.md)，我首先讲的是自顶向下的递归解法，实现了这样一个 `dp` 函数：
...
甚至更让人迷惑的是，有时候发现正向反向遍历都可以得到正确答案，比如我们在 [团灭股票问题](团灭股票问题.md) 中有的地方就正反皆可。
...
比如编辑距离这个经典的问题，详解见前文 [编辑距离详解](编辑距离.md)，我们通过对 `dp` 数组的定义，确定了 base case 是 `dp[..][0]` 和 `dp[0][..]`，最终答案是 `dp[m][n]`；而且我们通过状态转移方程知道 `dp[i][j]` 需要从 `dp[i-1][j]`, `dp[i][j-1]`, `dp[i-1][j-1]` 转移而来，如下图：
